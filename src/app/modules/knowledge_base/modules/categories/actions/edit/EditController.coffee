...
module.exports = class EditController extends ViewController
  ...
  initialize: (options) ->
    ...
    @setupRelatedCategory model, collection, view, formView
  ...
  getModel: (id, originalCollection) ->
    model = @appChannel.request 'kb:categories:entity', id
    ...
    @appChannel.request 'form:component', view,
      ...
      onFormDelete:  ->
        if window.confirm(i18n.t 'Are you sure you want to delete this?')
          categoriesChannel.trigger 'delete:kb:category', model
  ...
  formActionDone: (success = false) ->
    ...
    @region.empty()
    categoriesChannel.trigger 'done:kb:category'
  ...
  setupRelatedCategory: (model, categoriesCollection, view, formView) ->
    ...
    # First, filter out the current category to avoid setting itself
    # as its own parent category.
    # Make sure the collection is fetched before cloning.
    @appChannel.request 'when:fetched', categoriesCollection, ->
      filteredCollection = categoriesCollection.clone()
      filteredCollection.remove model

      view.availableCategories = filteredCollection
